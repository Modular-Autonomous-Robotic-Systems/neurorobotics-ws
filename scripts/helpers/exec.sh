...
iexec_ros_ap(){
			export $(grep -v '^#' $config_file | xargs)
			time docker container exec -it "$1" /bin/bash -l -c "source /opt/ros/${ROS_VERSION}/setup.bash && source /ardu_ws/install/setup.bash && source /ws/ros_ws/install/setup.bash && $2"
}
