...
build_image() {
    if [[ -z "$(docker images -q $2 2>/dev/null)" ]]; then
        if [[ "$(docker manifest inspect $2 >/dev/null 2>&1 && echo yes || echo no)" == "no" ]]; then
...
setup_nvim_config_for_image_build() {
    docker_dir=$1
    cp $NRT_WS/docker/dev/*.lua $docker_dir/
}

cleanup_nvim_config_after_image_build() {
    docker_dir=$1
    rm $docker_dir/*.lua
}
